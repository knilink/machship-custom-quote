var template = customFormular => `<div class="panel">
    ...
    <div class="panel-footer" ng-show="route.selected">
      <div class="route-search-result pull-left">
        <span class="route-price-dollar-sign">$</span><span class="route-price">{{${customFormular} | currency:""}}</span>
        </div>
...
var inject = template => '('+function(template) {
  'use strict';
  function trigger(){
...
chrome.storage.sync.get( function({formula}) {
  if(!formula) return;
  var s = document.createElement('script');
  s.innerText = inject(template(formula));
  s.onload = function() {
    this.remove();
  };
